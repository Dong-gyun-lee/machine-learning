...
def detect_outliers(data, data_list): 
    ...
    for col in data_list:
        ...
        Q1 = np.percentile(data2['승차인원수'], 25) 
        Q3 = np.percentile(data2['승차인원수'], 75) 
        IQR = Q3 - Q1 
        outlier_step = 1.5*IQR 
        outlier_list_col = data2[(data2['승차인원수'] < Q1 - outlier_step) | (data2['승차인원수'] > Q3 + outlier_step)].index 
...
for i in range(new_dict_frame.shape[0]):
    new_dict_number.append(len(new_dict_frame.at[i,'종교시설']))